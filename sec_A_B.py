...
for file in pdb_list:
    ...
    try:
        # # # dont need to determine chain because it only one in computed structures
        # url="https://data.rcsb.org/rest/v1/core/polymer_entity/" +file[0:4] +"/1"
        ...
        if os.path.exists("/home/dora/work/thesis/mammals_computed_structures/pdbfiles/")==False:
            pass
        else:
            n=0
            sec_str=""
            ...
            amino=""
            for l in fopen:
                #if l[11]==chain: there is only one chain here in computed structures
                if l[16]==" ":
                    char="-"
                    amino+=l[13]
                else:
...
os. chdir("/home/dora/work/thesis/mammals_computed_structures")
with open("d_per_A_B.txt", 'w') as f:
     f.write(json.dumps(d_per_A_B))
with open("list_A_prot.txt", 'w') as f:
     f.write(json.dumps(l_A_prot))
with open("list_B_prot.txt", 'w') as f:
